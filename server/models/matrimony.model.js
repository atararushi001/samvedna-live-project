...
const Matrimony = {
  ...
  create: (newUser, callback) => {
    const { profilePicture, email, password, dob } = newUser;

    db.query(
      `INSERT INTO ${db_name}.matrimony (email, password, dob) VALUES (?, ?, ?)`,
      [email, password, dob],
      (err, results) => {
        if (err) {
          return callback(err);
  ...
  getById: (id, callback) => {
    return db.query(
      `SELECT 
          ...
          COALESCE(ql.qualification_name, '') as qualificationName,
          COALESCE(es.education_specialization_name, '') as educationSpecializationName,
          COALESCE(cl.name, '') as currentLocationName,
          COALESCE(ql2.qualification_name, '') as partnerQualificationName
      FROM 
          matrimony m
      LEFT JOIN 
